...
/*		|| (1 == *(unsigned char *)&(const int){1})*/ \
	...
	#define isDWordLittleEndian()	(*(uint32_t *)&dwordOrder == 0x04030201)	/* These are slower routines for when little endianness cannot be guaranteed at compile time */
...
gdispImageError gdispImageOpen_BMP(gdispImage *img) {
	...
	if (offsetColorTable == 12+14) {
		...
		if (img->io.fns->read(&img->io, priv->buf, 40-4) != 40-4)
			goto baddatacleanup;
		// Get the width
		memcpy(&adword, (((uint8_t *)priv->buf)+0), sizeof(adword));
		CONVERT_FROM_DWORD_LE(adword);
		if (adword > 32768)				// This also picks up negative values
			goto unsupportedcleanup;
		img->width = adword;
		// Get the height
		memcpy(&adword, (((uint8_t *)priv->buf)+4), sizeof(adword));
		CONVERT_FROM_DWORD_LE(adword);
		...
		priv->bitsperpixel = aword;
		// Get the compression
		memcpy(&adword, (((uint8_t *)priv->buf)+12), sizeof(adword));
		...
		memcpy(&adword, (((uint8_t *)priv->buf)+28), sizeof(adword));
		CONVERT_FROM_DWORD_LE(adword);
