...
#include "ugfx/gwin/class_gwin.h"
...
/* Our default style - a green theme */
const GWidgetStyle GreenWidgetStyle = {
	HTML2COLOR(0xFFFFFF),			// window background

	// enabled color set
	{
		HTML2COLOR(0x00FF00),		// text
		HTML2COLOR(0x40FF40),		// edge
		HTML2COLOR(0xE0FFE0),		// fill
		HTML2COLOR(0xE0FFE0),		// progress - inactive area
	},

	// disabled color set
	{
		HTML2COLOR(0xC0FFC0),		// text
		HTML2COLOR(0x80FF80),		// edge
		HTML2COLOR(0xE0FFE0),		// fill
		HTML2COLOR(0xC0FFC0),		// progress - active area
	},

	// pressed color set
	{
		HTML2COLOR(0x40FF40),		// text
		HTML2COLOR(0x40FF40),		// edge
		HTML2COLOR(0x80FF80),		// fill
		HTML2COLOR(0x00FF00),		// progress - active area
	},
};

/* Our default style - a red widget theme */
const GWidgetStyle RedWidgetStyle = {
	...
	// enabled color set
	{
		...
		HTML2COLOR(0xFF4040),		// edge
		HTML2COLOR(0xFFE0E0),		// fill
		HTML2COLOR(0xFFE0E0),		// progress - inactive area
	},

	// disabled color set
	{
		HTML2COLOR(0xFFC0C0),		// text
		HTML2COLOR(0xFF8080),		// edge
		HTML2COLOR(0xFFE0E0),		// fill
		HTML2COLOR(0xFFE0C0),		// progress - active area
	},

	// pressed color set
	{
		HTML2COLOR(0xFF4040),		// text
		HTML2COLOR(0xFF4040),		// edge
		HTML2COLOR(0xFF8080),		// fill
		HTML2COLOR(0xFFE000),		// progress - active area
	},
};
...
void gwinSetText(GHandle gh, const char *text, bool_t useAlloc) {
	...
	if ((gh->flags & GWIN_FLG_ALLOCTXT))
		// Don't dispose if we still need memory and there is enough
		if (!useAlloc || (len < 1) || (len > gw->textAllocLen)) {
			gh->flags &= ~GWIN_FLG_ALLOCTXT;
			if (gw->text) {
				gfxFree((void *)gw->text);
				gw->text = "";
